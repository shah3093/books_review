                            ...
                            <table class="table table-hover" id="data-table-basic">
                                ...
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td>{{$user['email']}}</td>
                                        <td>{{$user['phone']}}</td>
                                        <td>{{ \App\Utils\CommonFunction::getUserType($user['user_type']) }}</td>
                                        <td>{{ \App\Utils\CommonFunction::getStatus($user['status']) }}</td>
                                        <td>
                                            @can(Config::get('constants.permissions.USER_EDIT'))
                                                <a href="{{route('user.edit',['id'=>$user['id']])}}"
                                                   class="btn btn-primary notika-btn-primary btn-icon-notika waves-effect"> Edit</a>
                                            @endcan
                                            @can(Config::get('constants.permissions.USER_DELETE'))
                                                <a href="#" data-href="{{route('user.delete',['id'=>$user['id']])}}"
